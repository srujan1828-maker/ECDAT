...
def build_plan(records, inputs):
    ...
    crypto = [2 + min(len(urgent), 10), 5 + min(len(urgent), 10) * 3]
    transfer = None
    ...
    phases = [
        ...
        {'id': 'crypto', 'track': 'cryptography', 'title': 'Upgrade and test cryptographic dependencies', 'days': crypto,
         'actions': ['Triage linked findings by usage and confidence; repair weak algorithms and certificate failures.', 'Inventory public-key dependencies and obtain vendor-supported post-quantum/hybrid upgrade paths.', 'Test client interoperability, handshake size, latency and rollback in staging; rescan code, binaries and TLS.'],
         'validation': 'Compatibility tests pass and evidence confirms intended changes; PQC remains unknown until negotiated groups are measured.', 'rollback': 'Retain a reviewed prior configuration; do not re-enable known weak cryptography as a routine rollback.'},
    ...
    if inputs.get('stack'):
        phases[1]['actions'].append('For the declared stack (' + inputs['stack'] + '), confirm runtime versions, build/start commands, ports, persistent paths and native dependencies on the target.')
    if traffic:
    ...
    return {'version': '1.0', 'status': 'draft', 'created_at': datetime.now(timezone.utc).isoformat(),
            ...
            'estimate': {'confidence': 'low', 'person_days': total, 'sequential_working_days': total,
                         ...
                         'formula': 'Hosting: [2+apps, 5+3×apps] + [3,8] for a database; unknown database adds 8 upper days; data >100 GB adds [2,5]. Crypto: [2+n,5+3n], n=min(urgent findings,10). Inventory and cutover: [1,3] each.',
